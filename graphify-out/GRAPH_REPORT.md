...
## Corpus Check
- 59 files · ~88,492 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 937 nodes · 1363 edges · 78 communities (48 shown, 30 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 127 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `21a69a97`
...
2. `run()` - 22 edges
3. `pipeline()` - 18 edges
4. `load_dashboard_state()` - 17 edges
5. `run_tool()` - 16 edges
6. `ContractToMvScriptTest` - 16 edges
7. `ValidateSegmentContractTest` - 15 edges
8. `run_mv()` - 13 edges
9. `_audio_duration()` - 13 edges
10. `run_orchestrator()` - 12 edges

## Surprising Connections (you probably didn't know these)
- `_run_pipeline()` --calls--> `resolve_python()`  [INFERRED]
  route.py → video_pipeline_core/platform_tools.py
- `run_orchestrator()` --calls--> `load_dashboard_state()`  [INFERRED]
  runtime.py → video_pipeline_core/dashboard_state.py
- `print_status()` --calls--> `load_dashboard_state()`  [INFERRED]
  runtime.py → video_pipeline_core/dashboard_state.py
- `rerun_node()` --calls--> `load_dashboard_state()`  [INFERRED]
  runtime.py → video_pipeline_core/dashboard_state.py
...
Cohesion: 0.09
Nodes (35): ContentQATest, Return the Python interpreter command for the current platform., resolve_python(), _cmd_init(), _cmd_new_run(), cmd_project_init(), cmd_project_new_run(), create_run_dir() (+27 more)
...
Cohesion: 0.11
Nodes (22): run(), cmd_burnsub(), cmd_concat(), cmd_cut(), cmd_download(), cmd_meta(), cmd_mksrt(), cmd_probe() (+14 more)
...
Cohesion: 0.15
Nodes (9): DashboardStateSpecTest, Generated request artifact uses the canonical object shape with items., 4. ComfyUI provider in build profile produces blocked/deprecated finding., 5. Existing route state.json dashboard mode still works., 2. Missing optional effects artifact does not fail when effects_enabled=false., 1. Manifest-based dashboard state includes required nodes., 3. Generated requests without generated manifest produce warn status on Fallback, load_dashboard_state() (+1 more)
...
Cohesion: 0.2
Nodes (18): adapt_contract_file(), _apply_stock_first_if_enabled(), contract_to_mv_script(), _hash_file(), _hash_json(), _load_category_ids(), _manifest(), contract_adapter.py — canonical-first runtime adapter(see roadmap.md).  公開 SPEC (+10 more)

### Community 12 - "Community 12"
Cohesion: 0.11
Nodes (7): MatchMontageMultiPickTest, MusicMixTest, MV-cut increment ①: beat → cut_grid (timeline:beat driver).  Pure-function tests, RunMvArtifactTest, ShotMidpointsTest, ShotStatsTest, SrtTsTest

### Community 13 - "Community 13"
Cohesion: 0.12
Nodes (17): _cjk_font(), compare_to_gold(), _extract_frame(), fixed_windows(), grid_durations(), _is_image(), plan_mv(), MV-cut — beat-driven timeline for the Unified Segment Model (timeline:beat).  In (+9 more)
...
Cohesion: 0.17
Nodes (13): video tools 統一錯誤。`fix_class`(material/spec/human)可選,讓 route/dashboard     像 Reco, ToolError, cmd_dashboard(), cmd_serve(), cmd_state(), cmd_story_map(), vt_dashboard.py — dashboard(監控:state.json/serve/self-contained 產生器), 從 video_too, 產生 self-contained story map HTML（route state 內嵌）。     免 server：直接用瀏覽器開檔即可看，不需 se (+5 more)
...
Cohesion: 0.15
Nodes (13): cmd_gen_bgm(), cmd_mix_audio(), 混合人聲 + BGM：BGM 降到指定音量，含淡入淡出, 合成一段情境氛圍墊樂（placeholder）。真曲可直接覆蓋 bgm/<mood>.mp3。, _audio_duration(), 用 ffprobe 取得媒體長度(秒)。共用底層原語。, cmd_assemble(), cmd_merge_final() (+5 more)

### Community 22 - "Community 22"
Cohesion: 0.15
Nodes (13): Find a CJK font file for ffmpeg drawtext / subtitle burning.      Returns the ab, resolve_font(), cmd_collage(), cmd_grade(), cmd_montage(), cmd_title_card(), cmd_title_sequence(), vt_effects.py — 特效師(色彩分級/字卡/Ken Burns/拼貼/蒙太奇),從 video_tools 解耦。 共用原語取自 vt_core,避 (+5 more)
...
Cohesion: 0.18
Nodes (11): beats_to_cut_grid(), detect_shots(), filter_shots(), _merge_short_spans(), Clean a shot list (contiguous (start, end) spans from detect_shots):     fold sh, (I/O) Split a long video into shots via PySceneDetect content detector.     Retu, 純函式:從 shots [(s,e)...] 算剪輯節奏指標(對照 66 期 gold standard 用)。     回 {n_shots, median,, Merge any (start, end) span shorter than min_len into its neighbour     (previou (+3 more)
...
Cohesion: 0.5
Nodes (4): _mv_result(), 純函式:驗 MV 劇本 v0 schema。回傳 {status, can_run, errors, warnings, issues}。     結訓核心:必, 純函式:驗 MV 劇本 v0 schema。回傳 {status, can_run, errors, warnings, issues}。     結訓核心:必, validate_mv_script()
...
## Knowledge Gaps
- **234 isolated node(s):** `MV 模式:route 驅動 canonical adapter 或 legacy mv_chain,寫 state.json 到 outdir。     (旁`, `印 MV state 的人工複核點(gap/必放/bookend)= 人在迴圈的接點。`, `找學員素材 seg{n}_user.* 或 seg{n}.*（圖或影片）。`, `MV 派工:無 state→跑 mv_chain;有 state→依 next_action 報告複核點。     MV 缺口(gap/必放/bookend)需`, `Resolve project directory and active/latest run directory.` (+229 more)
...
- **Why does `ToolError` connect `Community 19` to `Community 0`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 9`, `Community 16`, `Community 17`, `Community 18`, `Community 21`, `Community 22`, `Community 25`, `Community 26`, `Community 28`?**
  _High betweenness centrality (0.183) - this node is a cross-community bridge._
- **Why does `load_dashboard_state()` connect `Community 7` to `Community 1`, `Community 19`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `resolve_python()` connect `Community 1` to `Community 9`, `Community 23`?**
...
- **Are the 15 inferred relationships involving `load_dashboard_state()` (e.g. with `run_orchestrator()` and `print_status()`) actually correct?**
  _`load_dashboard_state()` has 15 INFERRED edges - model-reasoned connections that need verification._
