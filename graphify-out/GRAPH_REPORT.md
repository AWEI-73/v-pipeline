...
## Corpus Check
- 57 files · ~84,928 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 892 nodes · 1289 edges · 77 communities (47 shown, 30 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 113 edges (avg confidence: 0.78)
...
8. `compose_and_qa()` - 12 edges
9. `ValidateMvScriptTest` - 12 edges
10. `run_mv()` - 12 edges

## Surprising Connections (you probably didn't know these)
- `_run_pipeline()` --calls--> `resolve_python()`  [INFERRED]
  route.py → video_pipeline_core/platform_tools.py
- `cmd_search()` --calls--> `ToolError`  [INFERRED]
  video_tools.py → video_pipeline_core/vt_core.py
- `cmd_meta()` --calls--> `ToolError`  [INFERRED]
  video_tools.py → video_pipeline_core/vt_core.py
- `cmd_download()` --calls--> `ToolError`  [INFERRED]
  video_tools.py → video_pipeline_core/vt_core.py
- `cmd_probe()` --calls--> `ToolError`  [INFERRED]
  video_tools.py → video_pipeline_core/vt_core.py
...
Cohesion: 0.12
Nodes (20): ContentQATest, _is_windows(), platform_tools.py — cross-platform executable and resource resolver.  Centralize, Find yt-dlp executable., Find ollama executable., Return Ollama HTTP endpoint., Generic resolver: env → PATH → known locations → ToolError., Return the Python interpreter command for the current platform. (+12 more)

### Community 6 - "Community 6"
Cohesion: 0.12
Nodes (19): run(), cmd_concat(), cmd_cut(), cmd_download(), cmd_meta(), cmd_mksrt(), cmd_probe(), cmd_script_run() (+11 more)
...
Cohesion: 0.22
Nodes (17): adapt_contract_file(), _apply_stock_first_if_enabled(), contract_to_mv_script(), _hash_file(), _hash_json(), _load_category_ids(), _manifest(), contract_adapter.py — canonical-first runtime adapter(see roadmap.md).  公開 SPEC (+9 more)
...
Cohesion: 0.12
Nodes (7): MusicMixTest, MV-cut increment ①: beat → cut_grid (timeline:beat driver).  Pure-function tests, increment ③ I/O loop — frame extract + VLM scoring injected as fakes., RunMvArtifactTest, ScoreWindowsTest, ShotMidpointsTest, SrtTsTest
...
Cohesion: 0.13
Nodes (15): _cjk_font(), compare_to_gold(), _extract_frame(), grid_durations(), _mv_result(), plan_mv(), MV-cut — beat-driven timeline for the Unified Segment Model (timeline:beat).  In, (I/O) Grab a single frame at time t (seconds) → jpg. Returns bool ok. (+7 more)
...
Cohesion: 0.19
Nodes (14): cmd_verify(), _now_iso(), vt_verify.py — VERIFY(品管:5 維技術 QA + 內容對齊),從 video_tools 解耦。 共用原語取自 vt_core,避免循環匯, 維度1: 每個 script segment 是否都有對應的影片段落, 維度5: 解析度、framerate、有音軌、有視軌、無黑幀, VERIFY：對成片做 5 維度評分 + 路由修正指示, 維度2: 影片段落時長 vs TTS 時長，差值 < threshold, 維度3: SRT 內容 vs script.text 字元重疊率 (+6 more)
...
Cohesion: 0.18
Nodes (13): Find a CJK font file for ffmpeg drawtext / subtitle burning.      Returns the ab, resolve_font(), video tools 統一錯誤。`fix_class`(material/spec/human)可選,讓 route/dashboard     像 Reco, ToolError, cmd_title_card(), cmd_title_sequence(), 片頭/片尾：產生獨立的動態標題片段（文字滑入→hold→淡出）於深色底 + 暈影。     1920x1080/30fps/bt709。--anim slide, 字卡：在影片開頭疊加標題（淡入→hold→淡出），可選副標。保留規格與時長。 (+5 more)
...
Cohesion: 0.19
Nodes (12): cmd_pexels_download(), cmd_pexels_search(), _download_url(), fetch_stock_video(), _pexels_request(), _pexels_video_candidates(), _pixabay_video_candidates(), vt_stock.py — 素材來源:Pexels stock(服務 curator/素材腿),從 video_tools 解耦。 (+4 more)
...
Cohesion: 0.29
Nodes (9): _audio_policy(), build_assembly_plan(), build_timeline_build(), edit_artifacts.py — Node 9/10 build-facing JSON artifacts.  This module separate, Build Node 9 assembly_plan from generated MV payload., Write assembly_plan.json and optionally timeline_build.json., Build Node 10 timeline_build from concrete render plan clips., _snap_to_scene_cut() (+1 more)

### Community 31 - "Community 31"
Cohesion: 0.2
Nodes (10): allocate_segments(), detect_beats(), _plan_matched_segment(), _plan_stock_segment(), (I/O) Detect tempo + beat timestamps from an audio file via librosa.     Returns, 純函式:把音樂總長分給各段 → 每段幾個 clip、每 clip 多長。     montage/pace:fast → 多個快剪(~fast_clip 秒,預, local 段:用 match-mv 已配好的 clip 開窗(不 live 重評)。`_winfn` 可注入測試。, stock 橋段:Pexels 抓片(抓不到→GAP 不炸,可恢復)。`_fetch` 可注入測試。 (+2 more)

### Community 32 - "Community 32"
Cohesion: 0.2
Nodes (10): _default_scorer(), mv_chain(), Wrap content_qa.score_segment (lazy import) → fn(frame, desc)->(score,desc,reaso, increment ③ I/O: score each candidate window by grabbing its midpoint     frame, (I/O) increment ④：依 plan 從各原片抽段 → 統一 1920x1080 → 硬切拼接(對拍)     → 鋪音樂(trim 到視覺長度)→, 單一入口(roadmap #0 接線):material_db × 劇本 → match-mv → render。     把 curator 理解(capti, render_mv(), score_windows() (+2 more)
...
Cohesion: 0.25
Nodes (8): _drawtext_chain(), _mv_music_mix(), _photo_vf(), 靜照→影片片段的 vf。kenburns=緩慢推近(救開場/收尾照片,避免死板靜止);     否則純 hold(scale+pad)。輸出 1920x1080, 編劇文字層 → ffmpeg drawtext:narrative(全屏故事字卡:暗化+大字置中)+     label(底置中標籤)+ name_super(, 純函式:回 (filter_complex|None, audio_map)。audio_role 真混音(取代 v0 固定低音量):     - 有原音段(d, (I/O) render plan → 對拍/長段拼接 + audio_role 真混音。     keep_audio 段(duck/diegetic)保留原, render_mv_audio()

### Community 43 - "Community 43"
Cohesion: 0.29
Nodes (8): find_clips(), find_photos(), _plan_live_segment(), (I/O) 找 material_root 底下、路徑含 material_hint 的影片(scoping)。, (I/O) 找 material_root 底下、路徑含 material_hint 的照片(救開場/收尾 bookend)。, live 段:find_clips/photos → (照片 still) 或 (開窗→靜止預篩→VLM 評分→必放選段)。, Pick `n_slots` windows for an MV, **must-include-aware**（必放守門）.      candidates:, select_windows()
...
Cohesion: 0.4
Nodes (5): _asr_srt(), _burn_asr_subtitle(), (I/O) faster-whisper 轉錄 clip 原音 → SRT(時間軸相對 clip)。回 srt 或 None。     model_size 預, (I/O) 對講話段 clip 跑 ASR → 燒演講字幕(CJK)。回 subbed 路徑或 None。, _srt_ts()

### Community 71 - "Community 71"
Cohesion: 0.5
Nodes (4): audio_qa(), build_mv_state(), 純函式:MV 音訊搭配 QA(verify 的音訊維度,roadmap #6)。     檢查原音/音樂編排一致性——抓「該保留原音卻沒保留→被音樂蓋掉」這類, 把 MV 段落寫成 dashboard 認得的 state.json(放在 out_path 同目錄)。     純函式 → state dict;附帶寫檔。每

## Knowledge Gaps
- **211 isolated node(s):** `MV 模式:route 驅動 canonical adapter 或 legacy mv_chain,寫 state.json 到 outdir。     (旁`, `印 MV state 的人工複核點(gap/必放/bookend)= 人在迴圈的接點。`, `找學員素材 seg{n}_user.* 或 seg{n}.*（圖或影片）。`, `MV 派工:無 state→跑 mv_chain;有 state→依 next_action 報告複核點。     MV 缺口(gap/必放/bookend)需`, `Minimal .env loader (no external deps). KEY=VALUE per line; '#' comments.     D` (+206 more)
...
- **Why does `ToolError` connect `Community 20` to `Community 0`, `Community 32`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 7`, `Community 58`, `Community 42`, `Community 16`, `Community 17`, `Community 18`, `Community 23`, `Community 24`, `Community 26`, `Community 31`?**
  _High betweenness centrality (0.152) - this node is a cross-community bridge._
- **Why does `resolve_python()` connect `Community 5` to `Community 21`?**
