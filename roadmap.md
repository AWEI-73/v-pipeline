...
html_playwright ⏳ 本階段第二優先:HTML/CSS/JS 動畫(web 是最豐富的 2D motion
...
E3 P2 分配端補完:BUILD 切刀消費 attention_budget(現只有審計端),
   特效節奏與注意力預算同源。
E4 html_playwright MVP:一個配方(動態數字/infobox)走通
   HTML→Playwright→序列幀→overlay 全鏈,建立第二 backend 的 verify 模式。
E5 P4 收尾:CapCut GUI 載入驗證(人工閘)——text/audio 軌 draft 已寫好待驗。
