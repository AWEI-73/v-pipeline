...
<!-- TOOL_CONTRACT_START -->
{
  ...
  "canonical_tools": [
    ...
    },
    {
      ...
      "inputs": ["source video", "windows JSON from rough cut or Workbench selection", "output mp4 path", "highlight_cut_report.json path"],
      "outputs": ["stable H.264/AAC highlight mp4", "highlight_cut_report.json"],
      "stop_if": ["windows are empty or invalid", "ffmpeg re-encode fails", "output probe is missing H.264 video", "audio source exists but output AAC audio is missing"]
    }
  ],
  "supporting_tools": [
    {
...
- `tools/safe_highlight_cut.py`: renders accepted windows into a stable
  ...
  VP9/Opus, non-keyframe, or stutter-prone sources.
