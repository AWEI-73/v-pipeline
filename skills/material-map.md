...
<!-- TOOL_CONTRACT_START -->
{
  ...
  "canonical_tools": [
    ...
    {
      "tool": "tools/picture_plan_retrieval_report.py",
      "when": "Stage 5 has a picture plan and must prove that every selected material scene came from the canonical map ranker or an explicit evidenced override",
      "inputs": [
        "l1_picture_plan.json with retrieval_evidence",
      ...
      "stop_if": [
        "picture plan has no retrieval_evidence",
        "selected scene is not in the ranked candidates",
        "agent_override has no reason and evidence",
        "source hash does not match the canonical material map"
      ],
      "capability_id": "cap.material-map.picture-plan-retrieval-report.v1",
      "execution_class": "deterministic",
      "capability_role": "gate",
      "loops": [
        "L1"
      ...
      "maturity": "bounded",
      "certified_scope": "Stage 5 picture-plan retrieval evidence gate; report-only, no render"
    },
    {
      "tool": "tools/material_pool_map.py",
      "when": "shared material-pool checkout/commit is explicitly required by a multi-campaign run",
      "inputs": [
        "pool root and pool id",
        "campaign id",
        "campaign map for commit"
      ],
      "outputs": [
        "checked-out pool map or commit receipt"
      ],
      "stop_if": [
        "campaign base hash does not match",
        "operator expects this tool to perform visual review or selection"
      ],
      "capability_id": "cap.material-map.material-pool-map.v1",
      "execution_class": "deterministic",
      "capability_role": "operation",
      "loops": [],
      "maturity": "experimental"
...
## Picture-plan retrieval evidence (L1 gate)

The project map remains the single material truth. Before a Stage 5 picture plan
can be accepted, the plan must carry a `retrieval_evidence` object referencing
the canonical `project_material_map` and the output
`retrieval_ranking_report.json`. Run the registered gate:

```powershell
python tools\picture_plan_retrieval_report.py `
  --picture-plan RUN_DIR\stage5\l1_picture_plan.json `
  --segment-contract RUN_DIR\stage2\segment_contract.json `
  --project-map RUN_DIR\stage3\project_material_map.json `
  --out RUN_DIR\stage5\retrieval_ranking_report.json
```

The report replays `material_retrieval.rank_scenes` and records candidates,
selected scene IDs, rank positions, and source-hash checks. A clip outside the
declared top-K is allowed only when the clip itself carries
`selection_mode=agent_override` (or `owner_directed_override`) plus a reason
and evidence list. A filename/folder match is a weak prior for ranking, never
