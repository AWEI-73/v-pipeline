...
class EditDecisionRendererTest(unittest.TestCase):
    def _make_media(self, root):
        ...
        for command in (
            ["ffmpeg", "-y", "-f", "lavfi", "-i", "color=c=blue:s=320x180:d=1", "-c:v", "libx264", "-pix_fmt", "yuv420p", str(video)],
    ...
    def test_renders_canonical_decision_with_owned_manifest_and_streams(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            decision = {
                ...
                "cuts": [
                    {"id": "real_clip", "asset_id": "accepted_video", "source_type": "video", "timeline_in_sec": 0.0, "timeline_out_sec": 1.0, "in_seconds": 0.0, "out_seconds": 1.0},
                    {"id": "black_card", "source_type": "generated_background", "generated_background": {"color": "black"}, "timeline_in_sec": 1.0, "timeline_out_sec": 2.0},
                ],
            ...
            probe = subprocess.run(
                ["ffprobe", "-v", "error", "-show_entries", "stream=codec_type", "-of", "json", str(run / "final.mp4")],
                capture_output=True,
                text=True,
                check=False,
            )
