...
# Add the repo root to sys.path so we can import modules from video_pipeline_core
...
NODE_ORDER = ["0", "2", "3", "4-7", "5", "8", "9", "10", "11", "12", "13", "14"]


def _get_project_and_run(project_name=None):
    """Resolve project directory and active/latest run directory."""
    ...
    if active_path.exists():
        try:
            with active_path.open(encoding="utf-8") as f:
                ...
                active_project = active_data.get("active_project")
                active_run_rel = active_data.get("active_run")
        except Exception:
            pass
    ...
    if project_name:
        ...
        project_dir = project_root / active_project
    else:
    ...
    run_dir = None
    if project_name and active_project == slugify(project_name) and active_run_rel:
        run_dir = project_dir / active_run_rel
    elif not project_name and active_run_rel:
        run_dir = project_dir / active_run_rel
    else:
...
def _resolve_music_path(project_dir, run_dir, contract_path, args):
    ...
    try:
        ...
        if query and source == "yt":
            ...
            fetch_cmd = [
                python_exe, "video_tools.py", "music-fetch",
    ...
    while True:
        ...
        if next_action is None or next_action == "complete_review_final":
            ...
            if style == "mv":
                ...
                cmd = [
                    python_exe, "video_tools.py", "contract-run",
                    str(contract_path),
            ...
            if not blocking and state_json_exists(run_dir):
                # load state.json blocking
                try:
            ...
            if all_arrived and arrived_items:
                ...
                cmd = [
                    python_exe, "video_tools.py", "generated-manifest",
                    str(req_path),
...
def state_json_exists(run_dir):
    return (run_dir / "state.json").exists()
...
def print_status(project_name=None):
    ...
    print("-" * 80)
    print(f"Status: {'PASS' if run_info.get('pass') else 'FAIL'}".ljust(40) + f"QA Score: {run_info.get('score', 'N/A')}")
...
def rerun_node(node_label, project_name=None, args=None):
    ...
    if node_label == "12" and (run_dir / "final.mp4").exists():
        ...
        verify_cmd = [
            python_exe, "video_tools.py", "verify",
            "--script", script,
            "--timing", timing,
            "--edit-log", edit_log,
            "--srt", srt,
        ...
        state_cmd = [python_exe, "video_tools.py", "state", str(run_dir)]
