...
| Material Map | prove material truth, coverage, deltas, gaps, and rough-cut supply | `skills/material-map.md`, material-first acceptance tools, material lifecycle commands, `tools/source_section_map.py` / `tools/source_motion_profile.py` / `tools/source_material_matrix.py` before one-long-source highlight selection, `tools/source_dialogue_script.py` for interview/podcast transcript-safe cuts, `tools/source_highlight_plan.py` for accepted windows | `material_inventory_summary.json`, `materials_db.json`, `project_material_map.json`, `material_delta.json`, `source_section_map.json`, `source_motion_profile.json`, `source_motion_points.jpg`, `source_material_matrix.json`, `source_material_matrix_contact_sheet.jpg`, `source_soundtrack_probe_report.json`, `source_transcript.json`, `dialogue_edit_script.json`, `dialogue_highlight_windows.json`, `source_timeline_map.json`, `highlight_selection_plan.json`, `rough_cut_plan.json` | stop on inventory review, `await_map_review`, missing must-have needs, invalid maps, unreviewed generated assets, content-critical highlight windows without section/motion/matrix review evidence, or dialogue cuts that force duration over complete speech flow |
...
| Verify / Delivery | inspect outputs, evidence, and fail-closed delivery status | `skills/verify.md`, verify/reviewer tools, `tools/final_product_verify.py` for complete-video eye/ear evidence | verify reports, `final_product_verify_bundle.json`, review reports, delivery gate evidence | fail closed on missing evidence, stale artifacts, or hard-gate review failure |
