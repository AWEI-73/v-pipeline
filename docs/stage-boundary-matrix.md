...
| Story / Structure Planner | `story-soul-blueprint.md`, `writer.md`, `director.md`, method overlay `editorial-ambiguity-loop.md` | `story_soul_blueprint.json`, `screenplay_beats.json`, `story_decision_packet.json`, `director_shot_plan.json` | material truth, timeline, final render | story hypotheses are decided and causal beats state factual claim, entry/change/exit, evidence, and remaining unknowns | story lacks a causal spine or a route-changing/structural unknown remains open | Spec / Contract Compile |
| Spec / Contract Compile | `spec-contract.md`, `director.md`, method overlay `editorial-ambiguity-loop.md` | `segment_story_contract.json`, `evidence_need_map.json`, `stage2_ambiguity_gate_report.json`, `material_needs.json`, `segment_contract.json`, `effect_intent_plan.json` | material map truth, `final.mp4` | ambiguity gate exit 0 plus schema-valid canonical contract and needs | segment grammar/evidence needs are incomplete, hashes drift, or must-have needs are vague/untestable | Material Truth / Coverage |
