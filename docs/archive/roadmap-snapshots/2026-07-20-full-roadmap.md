---
title: Hermes Video Pipeline — Archived Full Roadmap
type: project
status: archived
updated: 2026-07-20
...
# Hermes Video Pipeline — Archived Full Roadmap

> Historical snapshot only. The active human summary is `roadmap.md`; agents
> enter through `RUNBOOK.md` and `HANDOFF_CURRENT.md`. Do not use this file as
> current route, status, or execution authority.
...
- New project runs declare `orchestration` artifacts in `run_layout.json` and
  initialize `route_orchestrator_state.json`, so a fresh session can resume the
  packet/state loop from the run root.
...
| MFL3 | Material-map rough-cut selector | Material review should feed the first edit, not stop at coverage truth. | **Complete 2026-06-23.** `video_pipeline_core/material_rough_cut.py` and `tools/material_rough_cut.py` turn accepted scene-to-need edges plus segment duration requests into `rough_cut_plan.json` and optional `timeline_build.json` without rendering. |
| MFL4 | Final review over rough timeline | Bad source repetition, off-topic footage, subtitle issues, and fatigue must surface before delivery. | **Complete 2026-06-23.** Delivery/dashboard gates now treat `rough_cut_plan.gaps` as blocking, continue to block bad source repetition from `timeline_build`, and surface caption/content/fatigue audit verdicts in final-review boundary state. |
| MFL5 | One real existing-material boundary pass | The route must prove itself on a real run folder after smaller gates pass. | **Complete 2026-06-23.** `tools/material_first_landing_case.py` builds a deterministic material-first run folder through intent, review apply, lifecycle, rough cut, timeline, editor review, final-review boundary, and stable `pipeline_home` Stage 5 cursor without rendering. |
