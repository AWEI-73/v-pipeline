...
| Verify / Delivery Gate | fail-closed review, final evidence, package/promote decisions | `verify_result.json`, `delivery_gate.json`, `video_only_delivery_waiver.json`, `final_product_verify_bundle.json`, `verified_preview_package.json`, `verified_preview_review_decision.json`, `story_human_review_decision.json` | missing/stale evidence, semantic gate fail, reviewer block, visible video-only limitation, unresolved human story review | repair owner or delivery promotion |
| Film Canon Product Route | registered film canon selection, pre-render story shell/catalog dry-run, product-route readiness gate | graduation artifacts or common `film_canon.json`, `film_blueprint.json`, `story_shell.json`, `catalog_map.json`, `review_packet.*`, `product_route_review_decision.json`, `reviewed_catalog_map.json`, `production_readiness_gate.json` | unknown film type, render requested, non-human approval treated as ready, pending review counted as missing, revision/rejected routed to production, missing human-confirmation flags | story/product review or production worker handoff |
