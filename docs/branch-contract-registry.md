...
- Route closure, docs/reference hygiene, and factory improvement-loop checks are
  machine facts. Do not replace them with narrative report claims.

## Branch Summary

| Branch | Owns | Key Outputs | Must Stop On | Returns To |
|---|---|---|---|---|
| Main Pipeline | Stage 0, route, BUILD eligibility, composition handoff, delivery promotion | `video_intent.json`, `segment_contract.json`, `delivery_requirements.json`, `render_handoff.json`, `final.mp4` | unresolved follow-up questions, unknown cursor, unresolved child contract, delivery gate block | child branches or delivery |
| Material Map | material truth, source understanding, scene-to-need edges, deltas, shot-level proof | `project_material_map.json`, `material_delta.json`, `source_material_matrix.json`, `dialogue_edit_script.json`, `rough_cut_plan.json`, `shot_level_material_proof_plan.json` | unreviewed scope, `await_map_review`, missing satisfies edges, unreviewed generated candidates | main route, story/structure, Workbench, Verify |
...
| Effect Factory | semantic effect translation, visual technique parameters, worker handoff, effect review | `visual_technique_plan.json`, `effect_contract.json`, `remotion_prompt_pack.json`, `remotion_effect_review.json`, `effect_handoff.json`, `effect_review.json` | unconfirmed parameters, missing required effect evidence, unsupported backend, generic output | main route, Workbench, Verify |
...
Use `tools\route_closure_integrity.py` to verify the route stage table,
branch registry, next-action vocabulary, and reviewer registry remain aligned.
Use `tools\doc_reference_hygiene.py` to classify top-level route/reference docs.
Use `tools\factory_improvement_loop.py` to turn rendered-QA/no-skip findings
into structured improvement backlog items without editing media or golden paths.
