...
| BSA1 soul-aware soft selection | `story_soul` + segment `core` soul fields + `director_intent` | `blueprint_to_contract.compile_contract`; `material_retrieval.rank_scenes` | `mv_cut._plan_local_segment` through map-ranked retrieval | `score_breakdown.soul` on ranked slots; on/off diff test proves same-tier selection can change | focused retrieval tests; soft evidence only | active (never admits zero-base evidence) |
