...
| 1 | Story Soul | What is the narrative device and emotional spine? | `story-soul-blueprint.md`, `blueprint-interview.md`, `writer.md` | `story-soul-blueprint`, `story-soul-to-contract`, `blueprint-coverage`, `blueprint-compile`, `blueprint-to-contract`, `tools/graduation_film_blueprint_catalog.py` | `story_soul_blueprint.json`, screenplay beats, director shot plan, `segment_contract.json` bridge, `graduation_film_canon.json`, `graduation_film_blueprint_A/B.json`, `story_shell_A/B.json`, `training_catalog_map.real_source.json`, `story_retarget_diff_A_to_B.json` | Stop if the output is just a parameter sheet with no conflict/turn/feeling, if graduation catalog assignments remain `pending_review`, or if required catalog modules are `missing`. | Intake / writer refinement |
...
| 7 | Verify | Does the output match story, material truth, subtitles, audio, and technical constraints? | `verify.md` | `verify`, `verify-evidence`, `final-product-verify`, `tools/timeline_review_packet.py`, `rendered_product_qa.py`, `timeline-audit`, `broll-audit`, `new-visual-audit`, `black-frame-audit`, `caption-audit`, `keyframe-grid`, `visual-audit`, `semantic-novelty-audit`, `action-progression-audit`, `replay-acceptance` | `verify_result.json`, `final_product_verify_bundle.json`, `timeline_review_packet.json`, `wall_index.json`, dense timeline walls, `rendered_product_qa.json`, audit reports, contact sheet, review report | Stop on tier-1 defects; `current_candidate` produces candidate flags, while `reference_film` produces non-blocking observations only. Both route uncertainty to targeted playback/crop, quality, or Brownfield work. | Material Truth or Brownfield Edit |
...
For any rendered rough cut/candidate or finished reference film, the horizontal
whole-story evidence surface is `tools/timeline_review_packet.py`: uniform walls make the complete
timeline readable to an agent, while optional soundtrack-probe and SRT bindings
let it flag audio/picture or subtitle/picture mismatches. It does not replace
deterministic QA and its semantic output cannot certify delivery. The caller must
declare `current_candidate` or `reference_film`; any bound SRT must declare its
text authority. Visible effects remain observations until a separate owner-reviewed
effect contract authorizes Effect Factory work.
...
| `film-canon-product-route` | registered film canon selection, product-specific story shell/catalog dry-run, visual-selection review/confirmation, pre-render readiness gate, locked graduation route trace harness, route-closure/doc-hygiene/factory-loop checks | `tools/film_canon_route.py`, `tools/write_product_route_review_decision.py`, `tools/film_canon_readiness.py`, `tools/graduation_film_blueprint_catalog.py`, `tools/visual_selection_gate.py`, `tools/write_visual_selection_review.py`, `tools/run_graduation_product_route.py`, `tools/route_closure_integrity.py`, `tools/doc_reference_hygiene.py`, `tools/factory_improvement_loop.py` |
