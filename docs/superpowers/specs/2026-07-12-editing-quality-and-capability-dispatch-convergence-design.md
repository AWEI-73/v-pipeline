...
任何全域索引、Domain view、LOOP view 或 Markdown 目錄都是 deterministic
generated view，不得成為第二份手工真相。
...
audio evidence 與 L5 packet 後，只停在 owner final taste verdict。
...
ID 是 join key，不使用自動遞增流水號。可另提供顯示編號，但不得拿它連線。
更名不改 ID；不相容契約才升 major version。
...
- `--query`：對 ID、tool、when、inputs、outputs 與 owner 做 deterministic
  token match；
- `--json`／`--out`：機器輸出；預設為精簡 human view。
...
9. generated catalog 與 live source 不一致；
10. supporting/internal/diagnostic tool 被宣稱為 Director public entry。

新增工具前，Agent 必須先查 `dispatch-capabilities --query`。若已有相近能力，
優先擴充既有 public surface；確實不相容時才新增，並以 `supersedes` 或
`overlaps_with` 留下關係。機器只能攔截結構重複與明確重疊；語意上是否真為
同一輪子仍由 Agent／reviewer 判斷。
...
6. 107/107 Python tools 仍有 owner，149 commands 仍全數分類；
7. orphan audit 的所有必要負向 fixtures 會 exit 1，真實 repo audit exit 0；
8. Director 只引用能力，不取得其他 Domain 的 canonical ownership；
9. focused＋adjacent tests、一次 full suite 與 `git diff --check` exit 0；
10. worker report 清楚分開 objective PASS、agent judgment、owner taste、
    music rights、creative approval 與 delivery claim；
11. `human_creative_approval=false` 與 `final_delivery_claimed=false` 保持，
    除非 owner 另行明文變更。
