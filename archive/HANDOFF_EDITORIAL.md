...
The converged SPEC?ender?erify pipeline made correct-but-lifeless videos. The
...
```
Mechanical-first (optional VLM later). Reviewer rules from spec 禮Node 12: report
...
def expand_shot_slots(segment: dict, n_required: int | None = None) -> list[dict]:
    ...
    #      "candidate_requirements"}]   (spec 禮Node 9 shape)
```
Pure; no rendering.
...
- `contract_adapter.py` (`run_contract`/around the build_profile build, ~line 460-520):
  ...
  (via `editorial_design`/spec 禮Node 8 defaults), put it into `build_profile_payload`.
...
**Task C ??wire editorial_qa (Task 4) into Node 12** using the 禮3 recipe (it's an
...
Task 4 + Task 5 (Gemini, parallel, isolated) ??Task A (activates visual_fatigue) ??Task C (wire 4/5) ??Task B ??Task D. After each, run the full suite and confirm the
